...
func client_setup() -> void:
	...
	if _table_list.is_empty():
		for _table in get_tree().get_nodes_in_group("table"):
			_table_list.append(_table)
...
func _seek_slot(_type: String) -> void:
	_seeking_for = _type
	match _type:
		"table":
			_table_index += 1
			if _table_index == _table_list.size() and on_sofa:
				update_state("go_away", Vector3.ZERO, Vector3(0, 0, -45))#!!!!!!!!!!!!!
				return
						
			if _table_index == _table_list.size():
				_seek_slot("sofa")
				return
			
			_table_list[_table_index].is_avaiable(self)	
		...
		"sofa":
			...
			if _sofa_index == _sofas_list.size():
				update_state("go_away", Vector3.ZERO, Vector3(0, 0, -45))
				return
			_sofas_list[_sofa_index].is_avaiable(self)

func update_state(_type: String,
				 _offset: Vector3 = Vector3.ZERO,
				 _position: Vector3 = Vector3.ZERO,
				_optional_rotation:float = PI ) -> void:
	match _type:
		"seek_table":
			_seek_slot("table")
		"seek_sofa":
		...
		"go_away":
			if _satisfied:
				_table_list[_table_index].get_node("Plate").get_node(_food['name'].to_pascal_case()).hide()
				_table_list[_table_index].change_avaiable_state(true)
			_update_food_state({})
...
func _physics_process(_delta: float)->void:
	...
	if Input.is_action_just_pressed("interact") and _character_ref != null and _food != {}:
		_has_food()
	...
	match _seeking_for:
		"table":
			if _distance > 0 and _distance <3:
				...
				_armature.get_node("panda").get_node("AnimationPlayer").play("Sitting_Start")
		...
		"sofa":
			if _distance > 0 and _distance <2:
				...
				global_position = _object_position
				_target_position = _object_position
				_armature.animate_action("Sitting_Start", _rotation)#especificar depois
				return
...
func configure_client_feedback(_item: Dictionary) -> void:
	_client_feedback.get_node("front").texture.load(_item["item_texture"])
	_client_feedback.get_node("back").texture.load(_item["item_texture"])

	globals.spawn_sfx("res://musics/sfx/assets/bell_ring.mp3", self)
	_update_food_state(_item)
	
func _update_food_state(_new_food: Dictionary) -> void:
	_food = _new_food
	if _new_food != {}:
...
func _on_interectable_area_body_entered(_body) -> void:
	if _body is character:
		_character_ref = _body


func _on_interectable_area_body_exited(_body) -> void:
	if _body is character:
		_character_ref = null

func _has_food() -> void:	
	var _inventory: Dictionary = _character_ref._inventory.get_inventory_aux()
	for _item in _inventory:
		if _inventory[_item]["item_name"] == _food["name"]:
			_table_list[_table_index].get_node("Plate").get_node(_food["name"].to_pascal_case()).show()
			
			var _item_price: int = _food["price"]
			_character_ref.update_gold(_item_price, "increase")
			globals.spawn_sfx("res://musics/sfx/assets/money.mp3", self)
			_character_ref._inventory.update_item(
				_inventory[_item]["item_name"], "decrease"
			)
...
func _on_waiting_timeout():
	if on_sofa:
		...
		_sofas_list[_sofa_index].change_avaiable_state(true)
		return
	on_table = false
	_update_food_state({})
