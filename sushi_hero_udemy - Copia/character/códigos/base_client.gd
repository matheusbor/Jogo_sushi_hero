	...
	match _seeking_for:
		"table":
			if _distance > 0 and _distance < 3:
				...
				_armature.get_node("panda").get_node("AnimationPlayer").play("Sitting_Start")#, _rotation
...
func _has_food() -> void:
	...
	for _item in _inventory:
		if _inventory[_item]["item_name"] == _food["name"]:
			...
			_armature.get_node("panda").get_node("AnimationPlayer").play("Sitting_Eating")#, _rotation
...
func _on_wait_timer_timeout() -> void:
	if on_sofa:
		on_sofa = false
		_table_index = -1
		_armature.get_node("panda").get_node("AnimationPlayer").play("Sitting_End")#, _rotation
