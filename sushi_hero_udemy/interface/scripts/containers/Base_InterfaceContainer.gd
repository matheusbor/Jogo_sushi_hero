...
func update_character(_type: String, _item: Dictionary, _update_type: String = "increase") -> void:
	match _type:
		"add":
...
func _process(_delta: float ) -> void:
	if get_tree().paused and not visible:
		return 
	if Input.is_action_just_pressed("ui_cancel") and visible:#para fechar a janela com uma ação padrão da godot
