...
func _seek_slot(_type: String) -> void:
	...
	match _type:
		"table":
			...
			_tables_list[_table_index].is_avaiable(self)	
	...
	match _seeking_for:
		"table":
			if _distance > 0 and _distance < 3:
				...
				_armature.animate_action("Sitting_Start")#especificar depois
				_client_feedback.get_node("AnimationPlayer").play("rotate")
...
func configure_client_feedback(_item: Dictionary) -> void:
	_client_feedback.get_node("front").texture = load(
		_item["item_texture"]
	)
	
	_client_feedback.get_node("back").texture = load(
		_item["item_texture"]
	)
...
func _has_food() -> void:
	...
	for _item in _inventory:
		if _inventory[_item]["item_name"] == _food["name"]:
			...
			_character_ref._inventory.update_item(_inventory[_item]["item_name"], "decrease")
			
			globals.spawn_sfx("res://musics/sfx/assets/eat.mp3", self)

			_armature.animate_action("Sitting_Eating")#especificar depois
...
func _on_waiting_timeout()-> void:
	if on_sofa:
		on_sofa = false
		_table_index = -1
		_armature.animate_action("Sitting_End", _rotation)#especificar depois
